...
import { Buttons, List, Location, Poll } from "./structures";
...
function createClient(clientName) {
    const client = new Client({
        ...
        //         '--no-zygote',
        //         '--disable-gpu',
        //         '--remote-debugging-port=9223',  // Cambié el puerto de depuración
        //         '--disable-extensions',
        //         '--single-process',
        //         '--disable-infobars',
        ...
        //     ],  // Aquí agregas las opciones
        //     executablePath: process.env.PUPPETEER_EXECUTABLE_PATH || '/usr/bin/google-chrome',
        //     timeout: 120000  // Aumentar el tiempo de espera
        // }
    });
...
function addListeners(client) {
    client.on('loading_screen', (percent, message) => {
        console.log(client.authStrategy.clientId + ' - LOADING SCREEN', percent, message);
    });
    ...
    client.on('qr', async (qr) => {
        ...
        console.log(client.authStrategy.clientId + ' - QR RECEIVED', qr);

        // paiuting code example
        const pairingCodeEnabled = false;
        if (pairingCodeEnabled && !pairingCodeRequested) {
            const pairingCode = await client.requestPairingCode('96170100100'); // enter the target phone number
            console.log(client.authStrategy.clientId + ' - Pairing code enabled, code: ' + pairingCode);
            pairingCodeRequested = true;
        }
    });

    client.on('authenticated', () => {
        console.log(client.authStrategy.clientId + ' - AUTHENTICATED');
    });
    ...
    client.on('ready', async () => {
        console.log(client.authStrategy.clientId + ' - READY');
        const debugWWebVersion = await client.getWWebVersion();
        console.log(`WWebVersion = ${debugWWebVersion}`);

        client.pupPage.on('pageerror', function (err) {
            console.log(client.authStrategy.clientId + ' - Page error: ' + err.toString());
        });
    ...
    client.on('message', async msg => {
        console.log(client.authStrategy.clientId + ' - MESSAGE RECEIVED', msg);
    ...
    client.on('group_join', (notification) => {
        ...
        console.log(client.authStrategy.clientId + ' - join', notification);
        notification.reply('User joined.');
    });

    client.on('group_leave', (notification) => {
        // User has left or been kicked from the group.
        console.log(client.authStrategy.clientId + ' - leave', notification);
        notification.reply('User left.');
    });

    client.on('group_update', (notification) => {
        // Group picture, subject or description has been updated.
        console.log(client.authStrategy.clientId + ' - update', notification);
    });

    client.on('change_state', state => {
        console.log(client.authStrategy.clientId + ' - CHANGE STATE', state);
    });
    ...
    client.on('call', async (call) => {
        console.log(client.authStrategy.clientId + ' - Call received, rejecting. GOTO Line 261 to disable', call);
        if (rejectCalls) await call.reject();
        await client.sendMessage(call.from, `[${call.fromMe ? 'Outgoing' : 'Incoming'}] Phone call from ${call.from}, type ${call.isGroup ? 'group' : ''} ${call.isVideo ? 'video' : 'audio'} call. ${rejectCalls ? 'This call was automatically rejected by the script.' : ''}`);
    });

    client.on('disconnected', (reason) => {
        console.log(client.authStrategy.clientId + ' - Client was logged out', reason);
    });
    ...
    client.on('message_reaction', async (reaction) => {
        console.log(client.authStrategy.clientId + ' - REACTION RECEIVED', reaction);
    });
