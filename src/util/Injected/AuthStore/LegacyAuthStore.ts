...
exports.ExposeLegacyAuthStore = (moduleRaidStr: any) => {
    ...
    // Intenta evitar eval y parsear la cadena como JSON si es posible
    try {
        moduleRaid = JSON.parse(moduleRaidStr);
    } catch (error) {
        // Si no es un JSON válido, usar eval como último recurso (aunque no es recomendado)
        eval('moduleRaid = ' + moduleRaidStr);
    }
