...
export const ExposeLegacyStore = () => {
    ...
    window.Store.QueryExist = (() => {
        const m = window.mR.findModule('queryExists')[0];
        return m ? m.queryExists : window.mR.findModule('queryExist')[0].queryWidExists;
    })();

    window.Store.ReplyUtils = (() => {
        const m = window.mR.findModule('canReplyMsg');
    ...
    const m = window.mR.findModule('ChatCollection')[0];
    if (m && m.ChatCollection && typeof m.ChatCollection.findImpl === 'undefined' && typeof m.ChatCollection._find !== 'undefined') {
